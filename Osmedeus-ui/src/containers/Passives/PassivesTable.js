...
export default class PayloadsTable extends Component {
  ...
  componentDidMount() {
    axios.get('http://127.0.0.1:8000/rest/api/passive/')
      .then(response => {
        this.setState({ rows: response.data.passives });
        ...
        cells: [
          {
            ...
            content: (
              <NameWrapper>
                ...
                <a href={window.location.pathname + "/" + item.passive_id}>
                  <Status text={item.passive_name} color={"purple"} />
                </a>
              </NameWrapper>
            ),
          },
          {
            ...
            content: (<Status text={item.analyze_type} color={"blue"} />)
          },
          {
