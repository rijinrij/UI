...
export default class PayloadsTable extends Component {
  ...
  componentDidMount() {
    axios.get('http://127.0.0.1:8000/rest/api/payloads/')
      .then(response => {
        this.setState({ rows: response.data.payloads, tamperRow: response.data.payloads });
        // this.setState({ tamperRow: response.data.payloads });
      })
      .catch(error => {
        // console.log(error);
    ...
    if (_.isEmpty(this.state)) {
      ...
      let tamperRow = this.state.rows.map((item, index) => ({
        ...
        cells: [
          ...
          {
            content: (
              <span><AkCode language="javascript" text={item.payload_string} /></span>
            ),
          },
          {
            content: (
